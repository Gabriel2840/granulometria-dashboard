...
def criar_mandala_svg():
    """Cria uma mandala SVG animada com cores Aura Minerals"""
    svg = """
    <div class="mandala-spinner">
        <svg viewBox="0 0 200 200" width="120" height="120" xmlns="http://www.w3.org/2000/svg">
            <!-- Circulo externo -->
            <circle cx="100" cy="100" r="95" fill="none" stroke="#1a3a52" stroke-width="2"/>
            <circle cx="100" cy="100" r="85" fill="none" stroke="#ff6b35" stroke-width="2"/>

            <!-- Circulos internos -->
            <circle cx="100" cy="100" r="70" fill="none" stroke="#1a3a52" stroke-width="1.5" opacity="0.7"/>
            <circle cx="100" cy="100" r="55" fill="none" stroke="#ff6b35" stroke-width="1.5" opacity="0.7"/>
            <circle cx="100" cy="100" r="40" fill="none" stroke="#1a3a52" stroke-width="1.5" opacity="0.7"/>
            <circle cx="100" cy="100" r="25" fill="none" stroke="#ff6b35" stroke-width="1.5" opacity="0.7"/>
            <circle cx="100" cy="100" r="10" fill="#1a3a52"/>

            <!-- Linhas radiais -->
            <line x1="100" y1="10" x2="100" y2="190" stroke="#ff6b35" stroke-width="1" opacity="0.5"/>
            <line x1="10" y1="100" x2="190" y2="100" stroke="#ff6b35" stroke-width="1" opacity="0.5"/>
            <line x1="30" y1="30" x2="170" y2="170" stroke="#1a3a52" stroke-width="1" opacity="0.5"/>
            <line x1="170" y1="30" x2="30" y2="170" stroke="#1a3a52" stroke-width="1" opacity="0.5"/>

            <!-- Triangulos decorativos -->
            <polygon points="100,15 110,35 90,35" fill="#ff6b35" opacity="0.8"/>
            <polygon points="100,185 110,165 90,165" fill="#ff6b35" opacity="0.8"/>
            <polygon points="15,100 35,110 35,90" fill="#1a3a52" opacity="0.8"/>
            <polygon points="185,100 165,110 165,90" fill="#1a3a52" opacity="0.8"/>
        </svg>
    </div>
    """
    return svg
...
def gerar_pdf_dashboard(ensaios):
    ...
    for ensaio in sorted_ens:
        ...
        amostra_page = ensaio.get('amostra', '').replace("á", "a")
        pdf.cell(0, 12,
                 f"{amostra_page} | {ensaio.get('data', '')} | Umidade: {ensaio.get('umidade', 0):.2f}%",
                 fill=True, ln=True, align="C")
        pdf.ln(4)
        df, total = calcular_granulometria(ensaio["massas"])
        if df is not None:
...
col_mandala, col_logo, col_titulo, col_hora = st.columns([0.8, 1.2, 3.5, 1])
with col_mandala:
    st.markdown(criar_mandala_svg(), unsafe_allow_html=True)
with col_logo:
    st.markdown(
...
with col_titulo:
    ...
    <div style='text-align:center;'>
        ...
        <p style='color:#ff6b35;font-size:13px;font-weight:600;margin:6px 0 0 0;'>✓ Agilidade e Simplicidade</p>
    </div>
    """, unsafe_allow_html=True)
with col_hora:
    st.markdown(
