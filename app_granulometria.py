...
def calcular_granulometria(massas_dict):
    ...
    total = sum(float(massas_dict.get(k, 0)) for k in ordered)
    if total == 0:
        return None, 0.0
    rows = []
    ac_retido = 0.0
    for ab in ordered:
        massa = float(massas_dict.get(ab, 0))
...
def _table_header(pdf, cw, hdrs, pr):
    ...
    for i, h in enumerate(hdrs):
        pdf.cell(cw[i], 8, h, fill=True, border=1, align="C")
    pdf.ln()
...
def _table_row(pdf, cw, row, pr, row_idx):
    ...
    pdf.cell(cw[0], 7, str(row["Malha (mm)"]), border=1, fill=True, align="C")
    pdf.cell(cw[1], 7, f"{row['Massa (g)']:.2f}", border=1, fill=True, align="C")
    pdf.cell(cw[2], 7, f"{row['Freq. Simples (%)']:.2f}%", border=1, fill=True, align="C")
...
def gerar_pdf_dados(ensaios):
    ...
    for ensaio in get_sorted(ensaios):
        ...
        pdf.cell(0, 8,
                 f"Amostra: {ensaio.get('amostra', '')}     "
                 f"Data: {ensaio.get('data', '')}     "
                 f"Umidade: {ensaio.get('umidade', 0):.2f}%",
                 ln=True)
        pdf.ln(3)
        ...
        pdf.cell(0, 8,
                 ...
                 "Aura Minerals - Beneficiamento Borborema",
                 align="C")
...
def gerar_pdf_dashboard(ensaios):
    ...
    pdf.cell(0, 15, "Aura Minerals - Dashboard Granulometria de Moagem",
             fill=True, ln=True, align="C")
    pdf.set_font("Helvetica", size=10)
    pdf.set_fill_color(50, 80, 110)
    pdf.cell(0, 8, "1025 TC 02  |  Beneficiamento - Borborema",
             fill=True, ln=True, align="C")
    pdf.ln(6)
    ...
    if sorted_ens:
        ...
        pdf.cell(0, 8, "INDICADORES-CHAVE (ULTIMO ENSAIO)", ln=True)
        ...
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Helvetica", size=10)
        pdf.cell(95, 7, f"Amostra: {ultimo.get('amostra', '')}")
        pdf.cell(0, 7, f"Data: {ultimo.get('data', '')}", ln=True)
        pdf.cell(95, 7, f"Umidade: {ultimo.get('umidade', 0):.2f}%")
        if df_u is not None:
            ret = get_freq(df_u, "50.8")
            fin = get_freq(df_u, "<6.35")
            pdf.cell(0, 7, f"Massa Total: {tot_u:.2f} g", ln=True)
            pdf.cell(95, 7, f"% Retido >50,8 mm: {ret:.2f}%")
            pdf.cell(0, 7, f"% Finos <6,35 mm: {fin:.2f}%", ln=True)
        pdf.ln(6)
    ...
    for i, e in enumerate(sorted_ens):
        ...
        pdf.set_fill_color(bg, bg, bg)
        pdf.cell(cw_tl[0], 7, e.get("amostra", ""), fill=True, border=1)
    ...
    cw = [32, 32, 44, 44, 44]
    hdrs = ["Malha (mm)", "Massa (g)", "Freq. Simples (%)", "Ac. Retido (%)", "Ac. Passante (%)"]
    for ensaio in sorted_ens:
        pdf.add_page()
        pdf.set_fill_color(*pr)
        pdf.set_text_color(255, 255, 255)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 12,
                 f"{ensaio.get('amostra', '')}  |  {ensaio.get('data', '')}  |  "
                 f"Umidade: {ensaio.get('umidade', 0):.2f}%",
                 fill=True, ln=True, align="C")
        pdf.ln(4)
        df, total = calcular_granulometria(ensaio["massas"])
...
with col_logo:
    st.markdown(
        '<div style="color:#ff6b35;font-weight:bold;font-size:20px;padding-top:10px;">'
    ...
    if not ensaios:
        ...
        with btn2:
            if PDF_OK:
                ...
                st.caption("fpdf2 nao instalado")
        with btn3:
            if PDF_OK:
                try:
        ...
        def ind_card(label, v_ant, v_atual, d_ant, d_atual, color):
            if d_ant:
                body = (
                    f"<div style='display:flex;align-items:center;gap:6px;'>"
                    f"<span style='font-size:18px;font-weight:500;color:{color};'>{v_ant}</span>"
                    f"<span style='font-size:13px;color:#aaa;'>→</span>"
                    f"<span style='font-size:18px;font-weight:500;color:{color};'>{v_atual}</span>"
                    f"</div>"
                    f"<div style='font-size:9px;color:#aaa;margin-top:3px;'>{d_ant} → {d_atual}</div>"
                )
            else:
                body = (
                    f"<div style='font-size:24px;font-weight:500;color:{color};'>{v_atual}</div>"
                    f"<div style='font-size:9px;color:#aaa;margin-top:3px;'>{d_atual}</div>"
                )
            return (
                f"<div style='background:white;border-radius:8px;"
                f"border:0.5px solid #e0e5ec;border-left:4px solid {color};"
                f"padding:10px 12px;height:80px;'>"
                f"<div style='font-size:9px;font-weight:500;color:#888;"
        ...
        with c1:
            st.markdown(ind_card(
                "% RETIDO &gt;50,8 MM",
                f"{ret_a:.1f}%" if anterior else "",
                f"{ret_u:.1f}%",
                data_a, data_u, SECONDARY,
            ), unsafe_allow_html=True)
        with c2:
            st.markdown(ind_card(
                "% FINOS &lt;6,35 MM",
                f"{fin_a:.1f}%" if anterior else "",
                f"{fin_u:.1f}%",
                data_a, data_u, "#1abc9c",
            ), unsafe_allow_html=True)
        with c3:
            st.markdown(ind_card(
                "MASSA TOTAL (G)",
                f"{tot_a:.2f}g" if anterior else "",
                f"{tot_u:.2f}g",
                data_a, data_u, "#2e6da4",
            ), unsafe_allow_html=True)
        with c4:
            st.markdown(ind_card(
                "N&deg; DE ENSAIOS",
                "", str(len(sorted_ens)),
                "", "Total registrado", PRIMARY,
            ), unsafe_allow_html=True)
        ...
        fig_c.update_layout(
            xaxis=dict(
                title="Abertura da Malha (mm)",
                type="log",
                tickvals=[3.18, 6.35, 9.52, 15.9, 25.4, 38.1, 50.8],
                ticktext=["<6,35", "6,35", "9,52", "15,9", "25,4", "38,1", "50,8"],
            ),
            yaxis=dict(title="Acumulado Passante (%)", ticksuffix="%", range=[0, 105]),
            hovermode="x unified",
            height=420,
            plot_bgcolor="rgba(240,244,248,0.5)",
...
with tab_reg:
    ...
    st.caption(
        "Preencha os campos abaixo. Freq. Simples, Ac. Retido e Ac. Passante "
        "sao calculados automaticamente a partir das massas informadas."
    )

    with st.form("form_ensaio", clear_on_submit=True):
        ...
        st.markdown("---")
        st.markdown("**Massa Retida por Peneira (g)**")
        st.caption("Insira a massa retida em cada peneira. Deixe 0 onde nao houve retencao.")

        massas_input = {}
        sieve_cols = st.columns(len(ABERTURAS))
        for i, ab in enumerate(ABERTURAS):
            with sieve_cols[i]:
                ...
                massas_input[ab] = float(val)

        submitted = st.form_submit_button(
            "Salvar e Calcular Ensaio",
            use_container_width=True,
            type="primary",
        )
    ...
    if submitted:
        if not amostra.strip():
        ...
        elif sum(massas_input.values()) == 0:
            ...
            df_novo, total_novo = calcular_granulometria(massas_input)
            st.success(
