...
def gerar_pdf_dashboard(ensaios):
    ...
    pdf.set_font("Helvetica", size=9)
    pdf.cell(0, 6,
             f"Relatório gerado: {datetime.now().strftime('%d/%m/%Y às %H:%M')}  |  "
             f"Total de ensaios: {len(sorted_ens)}",
             ln=True)
    pdf.ln(3)

    # Indicadores do último ensaio
    if sorted_ens:
        ultimo = sorted_ens[-1]
        ...
        pdf.set_font("Helvetica", "B", 10)
        pdf.cell(0, 7, "ÚLTIMO ENSAIO - INDICADORES-CHAVE", ln=True)
        ...
        pdf.text(x_col1, y_pos, f"Amostra: {ultimo.get('amostra', 'N/A')}")
